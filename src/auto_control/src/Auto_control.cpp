...
int main(int argc, char **argv)
{
    ...
    //创建名为node的智能指针对象
    auto node = rclcpp::Node::make_shared("AutoControl"); //创建节点
    ...
    auto control_modle_subscriber = node->create_subscription<fpv_msgs::msg::MoonlyFpv>("/control_model", 10, control_modle_subscriber_Callback);
    //订阅目标GPS位置信息
    auto aim_gps_subscriber = node->create_subscription<fpv_msgs::msg::MoonlyFpv>("/aim_gps", 10, aim_gps_subscriber_Callback);
    //发布FPV的期望动作
    auto act_pub_timer = node->create_wall_timer(std::chrono::milliseconds(10), act_pub);

    //使用线程来选择当前的自动操控逻辑模式
    std::thread thread_1(model_choose);
